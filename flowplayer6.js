...
common.tpl = function(elms, val) {
  var thumbs = elms.length ? elms : [elms];
  var seektime = document.createElement('div');
  var thumbnails = document.createElement('div');
  thumbs.forEach(function(elm) {
    if(!elm.hasChildNodes()) {
          elm.appendChild(thumbnails).setAttribute('class','fp-thumbnails');
          elm.appendChild(seektime).setAttribute('id','fp-seektime');
    } 
    document.getElementById("fp-seektime").innerHTML = val;
  });
};
...
flowplayer(function(api, root) {
   ...
   bean.on(root, 'mousemove', '.fp-timeline', function(ev) {
     ...
     common.tpl(timelineTooltip, format(seconds));
